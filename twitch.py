...
def startup():
    ...
    log.info("Setting up Stream Overlay")
    StreamOverlay.set_stream_overlay(StreamOverlay())
    asyncio.ensure_future(StreamOverlay.get_stream_overlay().start_stream_overlay("192.168.0.50", 12345))
...
def exit_handler(signal_number: int, _: FrameType):
    ...
    if StreamOverlay.get_stream_overlay():
        log.debug("Trying to stop Stream Overlay")
        asyncio.ensure_future(StreamOverlay.get_stream_overlay().stop_stream_overlay())
